...
import { randomCardType, randomOwnerId } from './utils';
...
describe('CardsService', () => {
  ...
  describe('create', () => {
    ...
    it('should return error in case owner id exist', async () => {
      try {
        let ownerId = randomOwnerId();
        let createCardDto: CreateCardDto = {
          ownerId,
        ...
        expect(
          await cardsService.createCard(createCardDto),
        ).rejects.toThrowError(new BadRequestException(CARD_EXIST));
        expect(getCardBySpy).toBeCalledWith(ownerId);
      } catch (error) {
        console.log('e', error);
      }
